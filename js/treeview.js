...
Tree View Javascript Class
=====================
Author : Harsh Chaurasia; Feb 2019

Used to create Tree View from JSON.
...
var jsonTree = (function () {
    ...
    function _NodeSimple(label, val, isLast) {
        ...
        labelEl.addEventListener('click', function (e) {
            ...
            if (e.altKey) {
                self.toggleMarked();
                try {
                    $('#tabletitle').text(e.target.innerText);
                } catch (e) { }
                return;
            }
    ...
    function _NodeComplex(label, val, isLast) {
        ...
        if (label !== null) {
            ...
            labelEl.addEventListener('click', function (e) {
                //debugger;
                try {
                    $('#tabletitle').text(e.target.parentElement.innerText);
                } catch (e) { }
    ...
    utils.extend(_NodeComplex.prototype, {
        ...
        fillGrid: function () {
            FillTableWithPayload(this.val, 'tablecontent');
...
/*
         * Created by Harsh-
         * 
         * Create Table and Tree from JSON, also binds them together.
         * 
         * @param payload {string} - JSON
         * @param tableID {string} - table container id
         * @param TreeWrapperID {string} - tree container id
         
 */
function FillTableWithPayload(payload, tableID) {
    payloadtable = Json2Html.convert(payload);
    // $('#tabletitle').text(Object.keys(payload)[0]);
    $('#' + tableID).hide(0, function () {
        $('#' + tableID).html(payloadtable).show(300);
    });

    $('table')
        .addClass('table table-striped')
        .css('border', '1px solid #ddd').each(function (index, _value) {
            try {
                var x = document.createElement("SPAN");
                x.className = "fa fa-arrow-circle-down";
                x.setAttribute("style", "float:right;cursor:default;padding:5px;");
                x.onclick = function () {
                    if (_value.style.display == "none") {
                        $(_value).show("fast");//.style.display = "";
                        x.className = "fa fa-arrow-circle-down"

                    }
                    else {
                        $(_value).hide("fast");//.style.display = "none";
                        x.className = "fa fa-arrow-circle-up"
                    }
                }
                //_value.rows[1].cells[_value.rows[1].cells.length - 1].appendChild(x)
                try {
                    if (_value.parentElement.parentElement.firstChild.outerHTML.indexOf("<th") == 0)
                        _value.parentElement.parentElement.firstChild.appendChild(x);
                }
                catch (e) {
                    e;
                }
...
function CollapseAllDataTable() {
    $("body").css("cursor", "progress");
    setTimeout(function () {
        $('.fa-arrow-circle-down').each(function (i, v) {
            try {
                v.click();
            } catch (e) { e; }
        });
...
function ExpandAllDataTable() {
    $("body").css("cursor", "progress");
    setTimeout(function () {
    $('.fa-arrow-circle-up').each(function (i, v) {
        try {
            v.click();
        } catch (e) { e; }
    });
